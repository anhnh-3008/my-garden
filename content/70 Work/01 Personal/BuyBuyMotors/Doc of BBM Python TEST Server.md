...
```sh
server {
    server_name test.buybuymotors.com;
    ...
    ssl_certificate /etc/letsencrypt/live/test.buybuymotors.com/fullchain.pem; # managed by Certbot
    ssl_certificate_key /etc/letsencrypt/live/test.buybuymotors.com/privkey.pem; # managed by Certbot
    include /etc/letsencrypt/options-ssl-nginx.conf; # managed by Certbot
    ssl_dhparam /etc/letsencrypt/ssl-dhparams.pem; # managed by Certbot
}

server {
    if ($host = test.buybuymotors.com) {
...
- Check log of Flask server: `server-logs`. This is alias of `sudo tail -f /var/log/buybuymotors_be/server.log`
